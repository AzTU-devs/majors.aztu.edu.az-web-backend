...
@router.put("/faculty/{faculty_code}")
async def update_faculty_endpoint(
    payload: UpdateFacultyManual,
    faculty_code: str = Path(...),
    db: AsyncSession = Depends(get_db),
):
    return await update_faculty_manual(faculty_code, payload, db)

@router.delete("/faculty/{faculty_code}")
async def delete_faculty_endpoint(
    faculty_code: str = Path(...),
    db: AsyncSession = Depends(get_db),
):
    return await delete_faculty_manual(faculty_code, db)
